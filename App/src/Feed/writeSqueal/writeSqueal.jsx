...
const WriteSqueal = () => {
  ...
  const handleNumeroInviiChange = (e) => {
    if (
      text.length * parseInt(e.target.value) +
        userData.caratteriGiornalieriUsati >
      userData.caratteriGiornalieri
    ) {
      alert(
        "Non hai abbastanza caratteri rimanenti per inviare il messaggio così tante volte"
      );
      return;
    }
  ...
  return (
    <div>
      <div className="header">
        <span className="x-step">
          <Link to="/Feed">x</Link>
        </span>
        <div id="post-squeal" onClick={handlePostSqueal}>
          Squeal
        </div>
      </div>
      ...
      {publicMode && (
        <div className="char-interface">
          <div className="characterUsed">
            ...
            <div
              className={noDailyCharsLeft ? "word-counterRed" : "word-counter"}
            >
              {text.length * numeroInvii + userData.caratteriGiornalieriUsati}/
              {userData.caratteriGiornalieri}
            </div>
          </div>
      ...
      <div className="automaticMessages">
        <div
          id="temporizzato"
          style={{
            color: isTemporizzato ? "green" : "red",
            border: `1px solid ${isTemporizzato ? "green" : "red"}`,
          }}
          onClick={toggleTemporizzato}
        >
          Send Timed Message
        </div>
        {isTemporizzato && (
          <div id="repetition-parameters">
            ...
            <input
              ...
              type="number"
              value={numeroInvii}
              onChange={(e) => handleNumeroInviiChange(e)}
              placeholder=""
            />
            <p> times</p> <p> every </p>
            <input
              id="input-temp"
              type="number"
              value={intervalloInvio}
              onChange={(e) => setIntervalloInvio(e.target.value)}
              placeholder=""
            />
            <p> min </p>
          </div>
